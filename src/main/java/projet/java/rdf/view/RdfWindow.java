...
public class RdfWindow extends JFrame {
	...
	/**
	 * Declaration des variables globale (les variables qui doivent �tre accecibles depuis les autres classes 
	 */
	...
	public RdfWindow(Viewer v) {
		...
		this.buttPrecedent =buttFactory("Pr�cedent",new Color(255, 255, 255), new Color(47, 204, 113),80,36);
	...
	/**
	 * creation de l'ent�te de la fen�tre
	 */	 
	private final JPanFactory createHeader(){
		JPanFactory panHeader = new JPanFactory(new Color(40,170,226),950,70); // creation du conteneur globale de l'ent�te
		...
		return panHeader;                                                      // retourn� le conteneur globale de l entéte
	}
	...
	private final JPanFactory createSideBar(){
		...
		JPanFactory panForm = new JPanFactory(new Color(25,36,46), 200, 110);  // sous conteneur qui sera positionn� en haut et contient tout les composante
		panForm.setLayout(new GridLayout(3,1,0,4));


		// boutton qui lance la recherche		
		JLabel labKeyWord = new JLabel("Mot-cl�");   //title
	...
	/**
	 * creation du tableau qui permet la navigation entre les diff�rentes modes
	 */
	...
	private final JTabbedPane createSwitchTable(){
		...
		jTab.setSelectedIndex(0); // positionn� le premier affichage a l'execution en mode recherche
		return jTab;
	}
	...
	public final  void Display() {
		this.setTitle("Projet Java - Master ACSIS - 2014/2015 Recherche par mots-cl�s dans"
				+ "des donn�es RDF (Groupe 4 : Rami - Amine - Ameur - Zakaria - Ania - Nadia)");
